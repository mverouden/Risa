...
<<label="LibraryPreload", echo=TRUE, eval=TRUE>>=
library(Risa)
require(faahKO)
@
...
<<label="Risa-readISAtab", echo=TRUE, eval=TRUE>>=
...
<<label="Risa-processXcmsSet", echo=TRUE, eval=TRUE>>=
 assay.filename <- faahkoISA["assay.filenames"][1]
 faahkoXset <- processAssayXcmsSet(isa = faahkoISA,
...
<<label="Risa-update", echo=TRUE, eval=TRUE>>=
...
<<label="Risa-write.assay.file", echo=TRUE, eval=TRUE>>=
temp <- tempdir()
...
%
<<label="pkgs", echo=FALSE, eval=TRUE, results=tex>>=
toLatex(sessionInfo())
@
%
